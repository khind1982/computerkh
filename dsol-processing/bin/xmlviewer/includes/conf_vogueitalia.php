...
$image_path = $os->join($image_path, substr(str_replace("/dc/vogueitalia-images/editorial","",$current_path), 0, -4));
$image_path = $os->join($image_path, '/images');
...
if (!file_exists($logFileName)) {
  $logFileHandle = fopen($logFileName, "w+") or die('unable to create logfile: '.$logFileName.'. Please run "chmod 777 /dc/'.$product[2].'-images/editorial" and try again.');
