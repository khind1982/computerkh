...
def marc_author_create():
    if len(eebxml.xpath('//rec/rec_search/author_main/author_corrected')) == 0:
        error_file.write('%s/%s: The main author name is missing from the <author_corrected> element. If correct, please ignore this message.\n' % (dirname, filename))
        marc_dict['author'] = None
...
def marc_245_create(author_element):
    ...
    if marc_dict['author'] is None:
        char_pos_00 = 0
    else:
        char_pos_00 = 1

    marc_dict['245'] = '%s%s%s$a%s' % ('  ', char_pos_00, char_pos_01, eeb_title)
...
#                auth_name = eebxml.xpath('//rec/rec_search/author_main/author_corrected')[0].text
                ...
                marc_author_create()
                if marc_dict['author'] is not None:
                    outf.write('%s\n' % (marc_dict['author']))
                marc_245_create(marc_author_create())
