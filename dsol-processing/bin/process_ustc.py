...
def create_lookup(xls, sheet):
    ...
    for rownum in range(sheet1.nrows):
        ...
        yr = str(sheet1.cell(rowx=rownum, colx=2).value)
        if '.' in yr:
            temp['year'] = yr.split('.')[0]
        else:
            temp['year'] = yr
                    ...
                    for line in in_data:
                        if line.strip().startswith('<place_of_publication>'):
                            ...
                            if data_lookup[pqid]['language'] != '':
                                ...
                                out.write(line)
                        elif line.strip().startswith('<startdate>'):
                            if data_lookup[pqid]['year'] is '':
                                out.write(line)
                            else:
                                match_str = str(data_lookup[pqid]['year'])
                                match = re.search('([0-9]{4})', match_str)
                                if match:
                                    new_date = '<displaydate>%s0101</displaydate>\n' % (match.group(1))
                                    out.write(new_date)
                                    if len(new_date) > 36:
                                        print new_date
                            ...
                            if data_lookup[pqid]['year'] is '':
                                ...
                                match = re.search('([0-9]{4})', match_str)
                                if match:
                                    new_date = '<displaydate>%s1231</displaydate>\n' % (match.group(1))
                                    out.write(new_date)
                                    #print new_date
                            ...
                            if data_lookup[pqid]['year'] is '':
                                ...
                                new_date = '<displaydate>%s</displaydate>\n' % (data_lookup[pqid]['year'])
                                #print data_lookup[pqid]['year']
                                out.write(new_date)
                            #print new_date
