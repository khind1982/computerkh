...
#my $mla_ab_matches = "/dc/elp/lionref/mla/data/20090206/matches.out";    # DEFINE WHERE LUT IS - FOR INSERTING ABELL LINKS
my $mla_ab_matches = "/dc/elp/lionref/data/master/rw_test/matches.out";    # DEFINE WHERE LUT IS
...
###nber of matches according to matchout
my $ftnums = 0;
my %abell_hash;
...
###ABBEL HASH CONTAIN THE RECORDS IN TARGET
###mla_file = file given at the prompt :ab_noft.txt
while ( $mla_file = shift ) {

    #my $out = '/home/richards/'. $mla_file . '.lin';
    my $out = $mla_file . '.lin';
    my $out2 = $mla_file . '.lnx';
    ###output file
    ...
    open( OUTX,      ">$out2" )      || die "Couldn't open MLA file: $mla_file\n$!\n";
    ## LOOKING AT LINES IN ab_noft.txt
    while (<MLA_RECS>) {
        my $mla_rec = $_;
        my $abell_rec;
        if ( $mla_rec =~ /<num>([^<]+)<\/num>/  ) {
            my $mla_num = $1;
            #GET THE NUMBER BETWEEN <num><num>
            my $abell_num;
            if ( exists( $lut_hash{$mla_num} ) && $mla_rec !~ /<ftnum>/ ) {
                ##IF WE HAVE THIS NUMBER USE AS KEY HAS A VALUE IN DICT IONNARY CREATED FROM MATCHOUT
                $abell_num = $lut_hash{$mla_num};
                #GET THAT VALUE: so looking at a <num><num> line  in ab-noft we found out that it is also in matchout as a match
                #look in the dictionary of record in TARGET an try to match the record in ab_noft to a record in TARGET
		   ...
		   #***
		    ...
		    #include <rpubdtl> as well as <pubdtl> in the records
                  ...
                  #if ( $mla_rec =~ /(<ISSN>.*?<\/ISSN>)/ ) {
                    if ( $abell_rec =~ /(<startend>.*?<\/startend>)/ ) {
			my $startend = $1;
			if ( $mla_rec =~ s/<\/ISSN>/<\/ISSN>\n$startend/ ) { }
			else { print LOG "Could not add <startend> $mla_num\n" }
			}
		    else {
			 print LOG "Could not find <startend> in ABELL $abell_num\n";
                    }
            ...
            ########CHANGE else { print OUT $mla_rec } Could print to another file print NOFT mla_rec
            else {print OUTX $mla_rec }
        }
