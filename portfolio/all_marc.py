...
def change_english_to_latin_008_field(record):
    field_008_text = record.get_fields('008')[0].value()
    field_008_text = re.sub('eng', 'lat', field_008_text)
    record.remove_fields('008')
    record.add_ordered_field(
        Field(
            tag='008',
...
def add_650_field_acta(record):
    """Add a new 650 field for Acta."""
    record.add_ordered_field(
        Field(
            tag='650',
            indicators=['\\', '0'],
            subfields=['a', 'Christian saints.']
        )
    )
    return record


def add_650_field_pld(record):
    """Add a new 650 field for PLD."""
    record.add_ordered_field(
        Field(
            tag='650',
            indicators=['\\', '0'],
...
def add_pqp_link(record, goid_lookup, product):
    """Given input MARC record, change value of $a
    subfield in the 856 field so it includes a link
    to the PQP record for the SSBE essay."""
    urlType = 'docview'
    if product == 'bsc':
        pqpName = 'Black Studies center'
    if product == 'acta':
        pqpName = 'Acta Sanctorum'
        urlType = 'publication'
    if product == 'pld':
        pqpName = 'Patrologia Latina'
        urlType = 'publication'

    goid = goid_lookup[record.get_fields('001')[0].value()]
    if goid == "product_level":
        urlSyntax = f'https://www.proquest.com/{product}'
    else:
        urlSyntax = f'https://www.proquest.com/{product}/{urlType}/{goid}'
        
    subfieldsLst = ['3', pqpName, 'u', urlSyntax]
    
    if record.get_fields('856')[0].get_subfields('s') and product != 'pld':
        sfield = record.get_fields('856')[0].get_subfields('s')[0]
        subfieldsLst.append('s')
        subfieldsLst.append(sfield)
...
def process_record(record, goid_lookup, product):
    ...
    if product == 'bsc':
        ...
        record = add_pqp_link(record, goid_lookup, product)

    if product == 'acta' or product == 'pld':
        record = update_date_stamp(record)
        record = change_leader_encoding(record)
        record = change_260_field_to_264_field(record)
        record = change_copyright_statement(record)
        record = add_pqp_link(record, goid_lookup, product)
        record = change_006_field(record)
        record = change_english_to_latin_008_field(record)
        if product == 'acta':
            record = add_650_field_acta(record)
        if product == 'pld':
            record = add_650_field_pld(record)
    return record
    ...
    with open(args.infile, 'rb') as inf, open(os.path.join(args.outdir, f'{os.path.basename(args.infile)}'), 'wb') as wr:
        ...
        for record in reader:
            ...
            if args.product == "gerritsen":
                ...
                output_record = process_record(record, goid_lookup, args.product)
...
def parse_args(args):
    ...
    parser.add_argument('product', help="Product code", choices=['bsc', 'gerritsen', 'acta', 'pld'])
    parser.add_argument(
