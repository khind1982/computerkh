<?xml version="1.0" encoding="UTF-8"?>
  <xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:opf="http://www.filemaker.com/fmpdsoresult" exclude-result-prefixes="opf">
    <xsl:output method="xml" indent="yes" />
    <xsl:template match="/opf:ROW">
      <document>
        ...
        <record>
          <film>
            ...
            <accnum>
              <xsl:value-of select="opf:AN"></xsl:value-of>
            </accnum>
            <xsl:if test="(place!='')">
                ...
                <xsl:if test="(CR1)">
                  <contributor>
                    ...
                    <originalform>
                      ...
                      <contributor_notes>
                        <production_role>Acknowledgement</production_role>
                      </contributor_notes>
                  </contributor>
                </xsl:if>
                <xsl:if test="(CRs)">
            ...
            <xsl:if test="(productions)">
              <productions>
                <production_details>
                  <xsl:if test="(opf:FI!='')">
                    <description>
                      <xsl:value-of select="opf:FI"></xsl:value-of>
                    </description>
                  </xsl:if>
                  <xsl:if test="(opf:SE!='')">
                    <production_series>
                      <xsl:value-of select="opf:SE"></xsl:value-of>
                    </production_series>
                  </xsl:if>
                </production_details>
              </productions>
              ...
              <notes>
                <xsl:if test="(opf:AR!='')">
                  <note>
                    ...
                    <xsl:value-of select="opf:AR"></xsl:value-of>
                  </note>
                </xsl:if>
                <xsl:if test="(opf:AH!='')">
                  <note>
                    ...
                    <xsl:value-of select="opf:AH"></xsl:value-of>
                  </note>
                </xsl:if>
                <xsl:if test="(opf:NH!='')">
                  <note>
                    ...
                    <xsl:value-of select="opf:NH"></xsl:value-of>
                  </note>
                </xsl:if>
                <xsl:if test="(opf:NT!='')">
                  <note>
                    <xsl:attribute name="type">Document</xsl:attribute>
                    <xsl:value-of select="opf:NT"></xsl:value-of>
                  </note>
                </xsl:if>
