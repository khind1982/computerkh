...
do
xmllint --noout --schema /dc/content_ops_management/project_toolkit/vendor_toolkit/data_spec_and_schema/leviathan.xsd ${f} 2>&1 | grep -v "validates" >> ./parsing-$batch.txt
done
...
LD_LIBRARY_PATH=/usr/local/lib python /dc/content_ops_management/Utils/empty-element-remove-refactored.py $valdir $batch
...
# cd /dc/${product}-images/samplings
