# Script to log incoming batches of Leviathan content, creating a tab delimited log file at issue level, counting pages and records
...
pubtitles = buildLut('/dc/content_ops_management/Utils/pmid-title.lut')
supplylist = '/dc/content_ops_management/Utils/content_supplied_todate.lut'
supplydict = buildLut('/dc/content_ops_management/Utils/content_supplied_todate.lut')

# Creates a list of all the content supplied to date
...
for nfile in locate('*', root=directory):
    ...
    if nfile.endswith('.pdf'):
        pdfnums.append((os.path.basename(nfile))[0:21])
...
for item in pdfnums:
    pdfcount = item, str(pdfnums.count(item))
    pdfnums2.append(pdfcount)

recnums2 = list(set(recnums2))
imgnums2 = list(set(imgnums2))
pdfnums2 = list(set(pdfnums2))
...
for item in issuelst:
    ...
    try:
        ...
        pass
        print '%s is not a recognised journal ID - please add it to the look up file in /dc/content_ops_management/Utils/pmid-title.lut' % re.split('/|_', item)[-4]
    ...
    pdfs = str(d3[issue]).translate(None, '[]\'')
    try:
        ...
        resupply = '\t'
    finallst.append(('%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\n') % (options.deldate, xmlbatch, options.batchnote, path, jid, pubtitle, issuedate, sourcetype, pages, records, pdfs, resupply))
...
with open(outfname, 'w') as outfile:
    for item in finallst:
        outfile.write(item)
print "Delivery log now available in %s" % outfname
